...
## 1.0.0-ra.78 "Low-Context Runtime" — 2026-09-16

RA now assumes every model is context-starved until proven otherwise. Nominal context windows are never trusted: the new context registry resolves each model's real usable window (config override > live probe > static table > conservative default), clamps it by the serving host's `num_ctx` ceiling, and the agent loop plans against that number. When tokens run low, runs get shorter and resume more often with the objective restated — the local-first contract from the RA-v2 design pack, plus the pack's missing piece: checkpoint/resume.

- **Context registry** (`anubis/src/context-limits.ts`): resolution for every model id — ra.json `provider.models.<id>.context` overrides, a live probe (`OllamaClient.showContext`: Ollama `/api/show` `*.context_length`, or `/v1/models` annotations on LM Studio-style servers, 60s cache), a curated static table (qwen3.8 → 262k nominal, gemma2 → 8k, llama3.1+ → 128k…), and conservative defaults (unknown local 8k / cloud 32k). The **usable** window further clamps the max by `context.server_cap` per provider (default local cap 32k — a "262k" qwen3.8 on a capped LAN box is a 32k model, full stop). Live proof from `ra doctor`: localhost `glm-5.2:cloud` probes at 1M nominal → clamped to 32k usable; `gemma:latest` probes at **8k**, overriding the table's 128k guess.
- **`num_ctx` correctness** (`anubis/src/ollama.ts`): native Ollama chats now send `options.num_ctx` (and `num_predict` for output caps) so the server actually reserves the window the registry promised — previously RA never set it and Ollama silently used its small server default, truncating prompts on "big-context" models. OpenAI-compat paths send `max_tokens`. The compaction summarizer also gets its own window-aware transcript cap + `num_ctx` (a 40k-char transcript no longer gets shoved into an 8k model).
- **Adaptive low-context runs** (`ra/src/agent.ts`, `ra/src/resume.ts`): a per-run token ledger tracks prompt pressure against the usable window, calibrated by real usage counts when the provider reports them (used = max(chars/4 estimate, real-based)). At the **low watermark** (70% default) the model is told to restate its objective + TODO state and wrap up — remaining steps cap at 2, subagent spawns disable. At the **critical watermark** (90%) or on a provider **context-overflow error** (new `isContextOverflowError` classifier — no longer a generic provider failure that burns the fallback chain and kills the run), the run checkpoints cleanly: a handoff packet (objective verbatim, files touched, TODO state, recent exchanges, next steps) persisted under `~/.ra/handoffs/` and returned as `status:"resumable"`. `runTaskAgent` then resumes a fresh, **shorter** run (step budget ×0.75, ×0.5…) seeded with the original objective + handoff — up to `context.resume_limit` (3) continuations. Still resumable after that: one escalation to a model with a larger usable window, reason recorded (`context exhausted after N continuations`) — or an honest `status:"partial"` result when nothing bigger is reachable (airgap). Kill switch: `RA_NO_ADAPTIVE=1`. Compaction is now repeatable and per-model instead of once-per-run at a fixed 60k chars.
- **Context-aware routing** (`anubis/src/capability.ts`): the mosaic skips models whose usable window is below the job floor (code/reasoning 16k, others 8k, raised by `context.min_context_tokens`) with the reason shown in `/providers` — this generically demotes any model whose real context can't hold the work (the "get rid of the 200k-context marketing" ask) without deleting profiles.
- **Surfaces**: `/context` TUI command (live pressure, per-model usable windows), `ra doctor` context-registry section, `/providers` + `ra providers` per-model `ctx` column, `ra eval --model <id>` sweeps with new columns (`ctx window·continuations·compactions·escalations`), and `ra benchmark run` now writes a JSON report per invocation (`RA-Tests/runs/report-*.json`) with per-scenario latency + the pipeline's last-run record — a crashed pipeline is a failed scenario, not a crashed benchmark.
- **Eval honesty fix**: `ra eval --model X` really tests X now — the role pin was being silently overridden by BOTH the profile tier table and the capability router, misattributing results to the wrong model (a 4/4 "gemma" sweep was actually cloud glm-5.2 doing the work). Eval now pins every tier + disables the mosaic for the run; the honest local baseline on this Mac's `gemma:latest` (8k usable) is 1/4 on the smoke subset.
- **Verification**: 30 new tests — anubis `tests/context-limits.test.ts` (resolution precedence incl. the o**llama**/llama substring trap, caps, ledger calibration, `nativeOptions`, overflow classifier, live probe via fake server, `num_ctx` in request bodies) + ra `tests/adaptive-run.test.ts` (handoff packet, shrinking step budget, and the full agent loop against a scripted fake Ollama: context-critical checkpoint → continuation seeded with the objective; overflow error → resume; exhausted continuations → honest partial). Full suites: 232 anubis (2 live skips, .251 down) + 272 ra, 0 fail. Live: `ra doctor`/`ra providers` against localhost Ollama; before/after eval sweep on `gemma:latest`; 4-tool competitive matrix re-run (see STATUS).

## 1.0.0-ra.77 "Provider Mosaic" — 2026-09-06

The multi-provider intelligence layer: log many AI endpoints and RA automatically routes each job to what each is best at, with quota-aware failover — local hardware does the cheap reasoning, cloud tokens get spent only on their specialty.

- **Capability profiles** (`anubis/src/profiles.ts`): every routable model carries benchmark-driven strengths (0–10 per job: code / research / reasoning / review / chat / docs), curated 2026-09 from SWE-bench Verified + Pro, Aider polyglot, and Humanity's Last Exam (GPQA is saturated). Gemini = the researcher (HLE ~46%, 1M context), Claude = the reviewer, GPT-5.6 = the frontier coder, DeepSeek V4 = the open all-rounder (80.6% Verified), GLM-5.2 = the cheap open coder, gpt-oss:120b = the local workhorse (~62% Verified on your own hardware). Unknown models get a mid-tier default.
- **Capability router** (`anubis/src/capability.ts`): maps each agent to its job (ptah→code, isis→research, maat/sekhmet→review, thoth→reasoning…) and picks the best-profiled available model across the whole pool — Ollama LAN/local/cloud, LM Studio (live discovery, cached 60s), and any configured OpenAI-compatible provider (Z.ai, Anthropic, OpenAI, Google…). Local models get a score bonus (`local_bonus`, default 2) implementing the "local assistant, cloud specialist" contract: cheap jobs stay on local hardware; a specialist cloud model wins only when clearly better. Pool allowlist (`capability_router.models`): missing/`["*"]` = every logged provider (full auto), explicit ids or `provider/*` prefixes constrain, lanes always in — the shipped config pins the stable lane pool plus `lmstudio/*`. `/providers` and `ra providers` show the live mosaic: every candidate with its profile, key status, and quota health.
- **Quota failover** (`anubis/src/quota.ts`): quota-style failures (429, rate limit, insufficient credits, billing) are now classified distinctly from auth errors. On a quota hit RA marks the provider exhausted for a 15-minute cooldown (`~/.ra/quota.json`), and the agent loop fails over to the next-best model for the job — cross-provider, re-picking the client per candidate, with visible attribution (`[RA fallback: … quota exhausted — using …]`). Auth failures and user cancellations still never fail over. The fallback order: user's explicit same-kind chain → cross-kind escalation (balanced/budget) → capability chain (next-best for the job).
- **First-class LM Studio**: provider block in ra.json + live model discovery feeding the routing pool — `lmstudio/qwen3-30b` competes on profile like any cloud model. Ollama local + LAN + Cloud were already core; they now carry profiles too.
- **Verification**: 21 new mosaic tests (profile matching, ranking with the local bonus and local-tie rule, quota classification/mark/expiry/reap, candidate enumeration with key presence, allowlist semantics incl. `provider/*` prefixes, failover chain shape, full-auto vs constrained pools) + a real-bug catch: the first implementation inferred "local" from the model string, wrongly bonusing every non-ollama-cloud provider — fixed by threading the authoritative candidate kind through the ranker. Full suites: 212 anubis + 263 ra green; `ra providers` verified against the live topology.

## 1.0.0-ra.76 "Agent Legion" — 2026-09-06

The biggest single release since the TUI: a 76-agent library, layered Mixture-of-Agents across local + cloud models, hybrid speed/quality routing with a budget guard, TUI upgrades (diff viewer, tables, telemetry, notifications), and four new tools including the skills runtime.

- **Agent library — 76 visible agents + 3 hidden system agents.** The 8 Egyptian core roles stay (pipeline compatibility); 65 pragmatic specialists join in 8 categories — research (repo-cartographer, api-archaeologist, dependency-scout…), implement (feature-builder, bug-fixer, test-writer…), review (code-reviewer, security-reviewer…), ops (release-manager, ci-fixer, docker-whisperer…), planning (architect, spec-writer, estimator…), domain (frontend-dev, database-eng, sre…), docs, data — every one a Markdown file with frontmatter permissions, model pinning, and ≤25-word descriptions (description-token budget lesson from Claude Code). Hidden system agents: `compaction` (folds long conversations into fidelity-preserving summaries once the prompt passes 60k chars), `title`, `summary`. New scoped registry `ra/src/agents/catalog.ts`: project `.ra/agents/` > user `~/.ra/agents/` > builtin, one name = one agent. `ra agents` lists grouped categories, `ra agents new <name>` scaffolds, `/agents` renders the catalog, and the palette's `agent:` entries now come from the library (previously config.agent only). The TASK tool hint embeds a token-budget-capped grouped catalog (≤1200 tokens, test-pinned) so small models can route to the right specialist.
- **Mixture-of-Agents 2.0 — layered, cross-model** (`ra/src/moa/layers.ts`, per arXiv 2406.04692). Layer 1 fans one prompt out across *models* (small-LAN + BIG-cloud by default), layer 2 critics (maat + sekhmet) see every proposal, then synthesis. Agreement matrix computes per-file consensus and surfaces disagreements; per-run cost cards show model, lane, ms, and estimated USD; pre-flight cost estimation aborts above `--budget` before any tokens burn (subscription lanes like Ollama Cloud glm estimate ~$0 honestly). Team presets in ra.json — `research-pack`, `security-sweep`, `refactor-squad`, `ship-it` — via `/team <name> <task>` or `ra moa --team`. Architect-class prompts now get an auto-suggest menu in the TUI: "Run as mixture across N models? est $X" (y/n; never silent spend). Classic role fan-out remains: `/moa <task> --roles thoth,maat`.
- **Persistent team boards (Cline-inspired).** `~/.ra/teams/<name>/{board.json, mailbox.jsonl, mission.log}` — task cards with status/model/ms, an append-only coordinator⇄specialist mailbox, and a mission log. `/board [name]` and `ra team list|status|mail NAME` inspect them; mixtures record every layer as cards for post-hoc audit and resume.
- **Hybrid local↔cloud routing (speed/quality).** Four modes via `routing.mode` or `RA_ROUTING`: `local-first` (default, unchanged), `quality-first` (everything but meta-tier to the cloud lane), `economy` (hard local-only), `balanced` (tier decides + cross-kind escalation: a failing local model may escalate to cloud, and cloud downshifts to local on budget breach). The same-kind fallback-chain contract is untouched — escalation is appended at the agent layer, never inside `fallbackChain`. New rolling latency cache (`~/.ra/latency.json`) feeds `/lane`'s new `routing:` + `latency:` lines and the degraded-local heuristic. Budget guard: `budget.session_usd` in ra.json; once crossed, cloud work downshifts to the local lane for the session with a visible notice.
- **TUI upgrades.** `/diff [file]` renders the checkpoint→current delta as a colored, hunk-headed unified diff (`@@` headers, +/- coloring, truncation guard) with `/undo` one keystroke away; `ra diff` parity kept. Markdown tables now render as aligned box-drawn tables that scale to terminal width. Every turn appends telemetry (time-to-first-token and tok/s). Desktop notifications (OSC 9 + BEL) on task completion, pref-gated (`~/.ra/tui.json` `notify`, `RA_NO_NOTIFY` to silence). Help/palette surface the new commands (`/team`, `/board`, `/diff`).
- **Four new tools** (same grammar, gated by the same permission engine): `SKILL name|list` — the skills runtime at last: RA reads the 22 vendored `.agents/skills/*/SKILL.md` (agentskills.io frontmatter, ancestors + `~/.ra/skills`, honoring `permission.skill`) and injects a skill's body into the conversation (PLAN.md Phase 1 item closed); `REPOMAP` — a token-budgeted repo overview ranked by git recency + symbol density (built on the existing symbols engine); `TEST [target]` — detects the runner (bun/pytest/vitest/jest direct, npm test otherwise) and executes it inside the sandbox with a parsed summary; `WEBSEARCH <query>` — Brave or Tavily keyed, airgap-aware, degrades to clear guidance without a key.
- **Verification.** Unit suites fully green after each epic: 247 ra tests (62 new across agents-catalog, moa-layers, tui-upgrade, tools-ra76) + 191 anubis tests (12 new routing cases). New pins: catalog integrity (≥70 visible agents, unique names, category floors, hint ≤1200 tokens), layered orchestration with injected fakes (cross-model fan-out, critics-see-everything, partial-failure semantics, budget abort before running), board persistence/resume, routing modes/escalation/budget math, diff hunk headers + truncation, table alignment, skills discovery/permissions, TEST/WEBSEARCH dispatch.
