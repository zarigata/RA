...
def catalog(d):
 ...
 check(re.search(r'RA 1\.0\.0-ra\.\d+',command(['ra','version'],d)['stdout']))
...
def pipeline_budget(d):
 c=json.loads(config_path.read_text());c['agent']['thoth']={'model':'ollama-cloud/glm-5.3-flash'}
 extra=cfgfile('pipeline-one-call',agent=c['agent'],agent_limits={'max_calls':1,'max_agents':8,'max_depth':2,'timeout_ms':120000})
 r=command(['ra','run','Create answer.py containing ANSWER=42. Planning stage: return only a one-sentence plan without any tool calls. Implementation stage: write the file.','--quick','--json'],d,extra)
 ...
 check(not last['filesWritten'] and last['models']==['glm-5.3-flash']);check('Stage ptah failed' in last['error'])
