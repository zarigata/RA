...
parser.add_argument('--guard-model',default='ollama-cloud/glm-5.3-flash')
...
(evidence/('summary-'+str(time.time_ns())+'.json')).write_text(json.dumps({'small_model':a.small_model,'review_model':a.review_model,'guard_model':a.guard_model,'results':[{'scenario':x['scenario'],'status':x['status'],'seconds':x['seconds'],'error':x['error']} for x in outcomes]},indent=2)+'\n')
